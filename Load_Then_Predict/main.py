...
def main():
    ...
    checkpoint_path = '/home/hujk17/TTS/WaveGAN/selftrain_zhaodan_v1_long/checkpoint-1000000steps.pkl'
    ...
    # model_class = getattr(
    #     parallel_wavegan.models, "ParallelWaveGANGenerator")
    model_class = getattr(
    ...
    with torch.no_grad():
        for idx, (utt_id, c) in enumerate(dataset, 1):
            print('now is', idx)
            # setup input
            x = ()
            if use_noise_input:
            ...
            sf.write(os.path.join(outdir, f"{utt_id}_gen.wav"),
                     y, config["sampling_rate"], "PCM_16")
            print('time predict:', time.time() - time_start)
            time_start = time.time()
