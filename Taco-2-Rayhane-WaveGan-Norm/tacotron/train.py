...
def model_train_mode(args, feeder, hparams, global_step):
	with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as scope:
		...
		if hparams.predict_linear:
			model.initialize(feeder.inputs, feeder.input_speaker_id, feeder.input_lengths, feeder.mel_targets, feeder.token_targets, linear_targets=feeder.linear_targets,
				targets_lengths=feeder.targets_lengths, global_step=global_step,
				is_training=True)
		else:
			model.initialize(feeder.inputs, feeder.input_speaker_id, feeder.input_lengths, feeder.mel_targets, feeder.token_targets,
				targets_lengths=feeder.targets_lengths, global_step=global_step,
				is_training=True)
		model.add_loss()
...
def model_test_mode(args, feeder, hparams, global_step):
	with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as scope:
		...
		model = create_model(model_name or args.model, hparams)
		if hparams.predict_linear:
			model.initialize(feeder.eval_inputs, feeder.eval_input_speaker_id, feeder.eval_input_lengths, feeder.eval_mel_targets, feeder.eval_token_targets,
			...
			if args.restore:
				...
				try:
					...
					if (checkpoint_state and checkpoint_state.model_checkpoint_path):
						...
						# print('ckpt steps is:', sess.run([global_step]))
						# # global_step = global_step * 0 + 1
						# assign_op = global_step.assign(1)
						# sess.run(assign_op)
						# # global_step = tf.assign(state,new_tensor)
						# print('set 1 then steps is:', sess.run([global_step]))
					...
					if hparams.predict_linear:
						...
						for i in tqdm(range(feeder.test_steps)):
							eloss, before_loss, after_loss, stop_token_loss, attention_loss, mel_p, mel_t, t_len, align = sess.run(
								[eval_model.loss, eval_model.before_loss, eval_model.after_loss, eval_model.stop_token_loss, eval_model.attention_loss, eval_model.mel_outputs[0], eval_model.mel_targets[0],
