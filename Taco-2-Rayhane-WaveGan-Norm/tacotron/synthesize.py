...
def run_eval(args, checkpoint_path, output_dir, hparams, sentences, speaker_id_lists):
	...
	speaker_id_lists = [speaker_id_lists[i: i+hparams.tacotron_synthesis_batch_size] for i in range(0, len(speaker_id_lists), hparams.tacotron_synthesis_batch_size)]
	...
	with open(os.path.join(eval_dir, 'map.txt'), 'w') as file:
		for i, texts in enumerate(tqdm(sentences)):
			...
			mel_filenames, speaker_ids = synth.synthesize(texts, speaker_id_lists[i], basenames, eval_dir, log_dir, None)
...
def tacotron_synthesize(args, hparams, checkpoint, sentences=None, speaker_id_lists=None):
	output_dir = 'tacotron_' + args.output_dir
	print('I want to see:', checkpoint)
	try:
	...
	if args.mode == 'eval':
		return run_eval(args, checkpoint_path, output_dir, hparams, sentences, speaker_id_lists)
