...
def write_metadata(metadata, out_dir):
	...
	mel_len = []
	for m in metadata:
		mel_len.append(int(m[4]))
	mel_len.sort(reverse=True)
	with open(os.path.join(out_dir, 'mel_distribute.txt'), 'w', encoding='utf-8') as f:
		f.write(str(mel_len) + '\n')
...
def norm_data(args):
	...
	supported_datasets = ['LJSpeech-1.0', 'LJSpeech-1.1', 'M-AILABS', 'Maybe_ObscureMoon5_22050', 'zhaodan_1w']
	if args.dataset not in supported_datasets:
		raise ValueError('dataset value entered {} does not belong to supported datasets: {}'.format(
			args.dataset, supported_datasets))

	if args.dataset.startswith('zhaodan_1w'):
		return [os.path.join(args.base_dir, args.dataset)]

	if args.dataset.startswith('Maybe_ObscureMoon5_22050'):
		return [os.path.join(args.base_dir, args.dataset)]
...
def main():
	...
	parser.add_argument('--dataset', default='zhaodan_1w')
